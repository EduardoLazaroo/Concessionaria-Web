...
"SElECT vendas.id, automoveis.modelo, clientela.nome, vendedor.nome nomev, vendas.data_venda FROM automoveis
JOIN vendas ON automoveis.id = vendas.idautomoveis
JOIN vendedor ON vendedor.id = vendas.idvendedor
JOIN clientela ON clientela.id = vendas.idclientela");
$vendass = $result->fetch_all(MYSQLI_ASSOC);
?>
...
<html lang="ptBR">
    ...
    <body>
        ...
        <section class="--style-content">
            <div>
                <a class="--list d-flex justify-content-start">Lista de Vendas</a>               
            </div>
            <table class="table ">
                <thead>
                    <tr>
                        <th class="--style-th" >#</th>
                        <th class="--style-th" >Automovel</th>
                        <th class="--style-th" >Vendedor</th>
                        <th class="--style-th" >Cliente</th>
                        <th class="--style-th" >Dia da Venda</th>
                ...
                <tbody>
                    <?php foreach ($vendass as $vendas) { ?>
                        <tr class="--style-tr">
                            ...
                            <td><?php echo $vendas["modelo"]; ?></td>
                            <td><?php echo $vendas["nomev"]; ?></td>
                            <td><?php echo $vendas["nome"]; ?></td>
                            <td><?php echo date("d/m/Y", strtotime($vendas["data_venda"])); ?></td>                       
                            <td class="--button-delete btn btn-danger">
                                <?php echo "<a href='exclui_vendas.php?id={$vendas['id']}'>Excluir</a>"; ?>
                            </td>
